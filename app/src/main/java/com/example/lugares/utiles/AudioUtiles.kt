...
class AudioUtiles(
    ...
    init {
        btAccion.setOnClickListener { grabaStop() } //cuando se le de click haga eso
    ...
    private fun grabaStop() {
        if (ContextCompat.checkSelfPermission(contexto, Manifest.permission.RECORD_AUDIO) != //para ver si tengo los permisos de grabar
    ...
    private fun iniciaGrabacion() {
        try {
            ...
            btPlay.isEnabled = false
            btDelete.isEnabled = false
        } catch (e: IllegalStateException) { //para captar errores
            e.printStackTrace()
        } catch (e: IOException) {
            e.printStackTrace()
        }
    ...
    private fun detenerNota() { //para cuando no esta grabando
        btPlay.isEnabled = true
        btDelete.isEnabled = true
        mediaRecorder?.stop()
        mediaRecorder?.release()
        Toast.makeText(contexto,msgDetieneNotaAudio,Toast.LENGTH_SHORT).show()
        btAccion.setImageResource(R.drawable.ic_mic) //cambia la imagen del boton
    }

    private fun playNota() { //para ver si existe y si puede ser leido
        try {
            if (audioFile.exists() && audioFile.canRead()) {
                val mediaPlayer = MediaPlayer()
                mediaPlayer.setDataSource(audioFile.path) //para saber donde esta el archivo
                mediaPlayer.prepare()
                mediaPlayer.start()
            }
        } catch (e: IOException) {
